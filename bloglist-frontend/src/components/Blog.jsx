...
const Blog = ({ blogs, setBlogs, user}) => {
  ...
  const handelLike = async ()=> {
    ...
    setBlogs(blogs.map(blog => (blog.id||blog._id) === id ? updated : blog ))
  ...
  const isOwner = () => {
    ...
    return String(blogUserId) === String(userId) || blogUserName === user.userName
  }
  ...
  return(
    <>
      <Paper elevation={0} sx={{ p: 3, border: '1px solid #e0e0e0', borderRadius: 2, maxWidth: 600 }}>
        ...
        <Box sx={{ mb: 1 }}>
          <MuiLink href={blog.url} target="_blank" rel="noopener noreferrer" underline="always">
            {blog.url}
          </MuiLink>
        </Box>
